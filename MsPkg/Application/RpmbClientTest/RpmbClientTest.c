...
MODULE ("EFI_RPMB_IO_PROTOCOL Client Test");
...
static UINT8 RpmbAuthKey[EFI_RPMB_PACKET_KEY_MAC_SIZE] = {
  ...
  )
{
  ...
  if (RequestOrResponseType == EFI_RPMB_REQUEST_PROGRAM_KEY) {
    memcpy (Packet->KeyOrMAC, RpmbAuthKey, sizeof (RpmbAuthKey));
  }
}
  ...
  )
{
  ...
  LOG_COMMENT ("RpmbTestProgramKey\n");

  GeneratePacket (EFI_RPMB_REQUEST_PROGRAM_KEY, &Request);
  GeneratePacket (0, &ResultResponse);

  VERIFY_SUCCEEDED (
    RpmbProtocol->ProgramKey (
      RpmbProtocol,
      &Request,
      &ResultResponse),
    "Verify programming of the authentication key");

  DumpPacket (&Request);
  DumpPacket (&ResultResponse);

  RpmbVerifyResponseStatus (
    &ResultResponse,
    EFI_RPMB_RESPONSE_PROGRAM_KEY,
    EFI_RPMB_OK);
}
  ...
  )
{
  ...
  VERIFY_SUCCEEDED (
    RpmbProtocol->AuthenticatedRead (
      RpmbProtocol,
      &ReadRequest,
      &ReadResponseBuffer),
  ...
  )
{
  ...
  VERIFY_SUCCEEDED (
    RpmbProtocol->AuthenticatedWrite (
      RpmbProtocol,
      &WriteRequestBuffer,
      &WriteResponse),
...
VOID
RpmbTestSingleWriteRead (
  VOID
  )
{
  UINT8 WriteData[EFI_RPMB_PACKET_DATA_SIZE];
  EFI_RPMB_DATA_PACKET Request;
  EFI_RPMB_DATA_BUFFER RequestBuffer;
  EFI_RPMB_DATA_PACKET Response;
  EFI_RPMB_DATA_BUFFER ResponseBuffer;

  RandomBytes(sizeof(WriteData), WriteData);
  GenerateDataPacket (
    WriteData,
    sizeof (WriteData),
    0,
  ...
  GeneratePacket (EFI_RPMB_REQUEST_AUTH_READ, &Request);
  GeneratePacket (0, &Response);
  ...
  VERIFY_IS_TRUE(
    AreEqualBytes(WriteData, Response.PacketData, sizeof(WriteData)),
    "Verify that read data matches written data");
}

VOID
RpmbTestSequentialWriteRead (
  VOID
  )
{
  UINT8 WriteData[EFI_RPMB_PACKET_DATA_SIZE];
  UINT8 ReadData[EFI_RPMB_PACKET_DATA_SIZE];

  UINTN RpmbStorageSize = (UINTN)RpmbProtocol->RpmbSizeMult * (UINTN)(128 * 1024);
  UINTN TotalPacketCount = RpmbStorageSize / EFI_RPMB_PACKET_DATA_SIZE;
  UINTN Lba;
  UINT64 TotalWriteElapsedTime = 0;
  UINT64 TotalReadElapsedTime = 0;
  ...
  for (Lba = 0; Lba < TotalPacketCount; ++Lba) {
...

    {
      ...
      // Read Packet
      //
      TotalReadElapsedTime += RpmbRead (Lba, sizeof (ReadData), ReadData);
    }

    VerifyAreEqualBytes (WriteData, ReadData, sizeof(WriteData));

    SET_LOG_LEVEL (TestLogComment);
  }

  LOG_COMMENT (
    "Block Write Avg Time: %lldus, Block Read Avg Time: %lldus\n",
    (TotalWriteElapsedTime * 1000ULL) / TotalPacketCount,
    (TotalReadElapsedTime  * 1000ULL) / TotalPacketCount);
}

VOID
RpmbTestReadPerformance (
  VOID
  )
{
  UINTN RpmbStorageSize = (UINTN)RpmbProtocol->RpmbSizeMult * (UINTN)(128 * 1024);
  ...
  ASSERT (mBuffer == NULL);
  mBuffer = (UINT8 *) AllocateZeroPool (RpmbStorageSize);
  VERIFY_IS_NOT_NULL (mBuffer);

  //
  // Read Packet
  //
  TotalReadElapsedTime = RpmbRead (0, RpmbStorageSize, mBuffer);

  LOG_COMMENT (
    "TotalReadElapsedTime: %lldms, Block Read Avg Time: %lldus\n",
    TotalReadElapsedTime,
    (TotalReadElapsedTime * 1000ULL) / TotalPacketCount);
  ...
  )
{
  ...
  LOG_COMMENT ("CID: ");
  HexDump (RpmbProtocol->Cid, EFI_RPMB_CID_SIZE);
  ...
  )
{
  ...
  TEST_FUNC (RpmbTestProgramKey);
  TEST_FUNC (RpmbTestSingleWriteRead);
  TEST_FUNC (RpmbTestSequentialWriteRead);
  TEST_FUNC (RpmbTestReadPerformance);
